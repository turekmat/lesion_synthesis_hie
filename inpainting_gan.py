...
class BiasedMAELoss(nn.Module):
    ...
    Zároveň penalizuje i příliš nízké hodnoty blízké nule.
    """
    def __init__(self, weight=5.0, high_penalty_factor=2.0, neighbor_radius=2, min_threshold=0.05, low_penalty_factor=2.0):
        ...
        self.neighbor_radius = neighbor_radius  # Radius pro výpočet průměrných hodnot okolí
        self.min_threshold = min_threshold  # Minimální přijatelná hodnota (prevence hodnot blízkých nule)
        self.low_penalty_factor = low_penalty_factor  # Penalizace pro příliš nízké hodnoty
    ...
    def forward(self, pred, target, mask, orig_range=None):
        ...
        # NOVÉ: Identifikace míst, kde předpověď je příliš blízko nule
        too_low_values = (pred < self.min_threshold) * binary_mask
